...
import ru.szhuvertsev.itmo.sd.stock.service.UserService
...
class UserController(private val userService: UserService, private val stockService: StockService) {
    ...
    fun createUser(@RequestBody body: String): ResponseEntity<Json> {
        ...
        val newUser = userService.registerUser(login, BigDecimal(initialBalance))
    ...
    fun increaseUserBalance(@RequestBody body: String) {
        ...
        val valueToIncrease = bodyObj.valueToAdd ?: "0"
    ...
    fun sellShares(
        ...
        try {
            userService.increaseUserBalance(
                userId,
                stockService
                    .getCompanySharesPrice(companyId)
